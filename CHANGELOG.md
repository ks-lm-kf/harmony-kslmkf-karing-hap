...
- VPN 接口固定创建 `0.0.0.0/0` 和 `::/0` 全设备路由，并移除部分设备不兼容的动态 `vpnId`。
- 启动前清理重复 FakeIP DNS、修复 DNS 引用，并把 Clash API 恢复到 `127.0.0.1:3057`。
- 删除当前配置后立即同步代理页面，避免仍显示已删除配置的节点。
- 订阅更新完成后立即刷新更新时间、状态与节点列表；更新失败时恢复原配置和选中节点。
- 状态栏使用独立纯色背景，不再叠加页面模糊、渐变或液态玻璃滤镜。
- API 23/24 模拟器完成 VLESS、Trojan、SS 订阅导入与双栈 VPN 启动路径回归。
- API 26 arm64 真机验证核心 API 在线、真实流量统计、IPv4/IPv6 代理出口、境外站点延迟和 VPN 停止清理。
